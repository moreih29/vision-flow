...
async def upload_images(
    ...
    await dataset_service.get_dataset(db, dataset_id)
    ...
    for i, file in enumerate(files):
        ...
        image = await image_service.upload_image(
            db, dataset_id, current_user.id, file, storage, folder_path=fp
        )
...
@router.get(
    "/datasets/{dataset_id}/folders", response_model=FolderContentsResponse
)
async def get_folder_contents(
    ...
    """List folders and images at a given folder path within a dataset."""
    await dataset_service.get_dataset(db, dataset_id)
    return await image_service.get_folder_contents(
        db, dataset_id, path, skip, limit
    )


@router.get(
    "/datasets/{dataset_id}/images", response_model=ImageListResponse
)
async def list_images(
    ...
    images, total = await image_service.get_images_by_dataset(
...
@router.patch("/datasets/{dataset_id}/folders", status_code=200)
async def update_folder(
    ...
    count = await image_service.update_folder_path(
        db, dataset_id, body.old_path, body.new_path
    )
    return {"updated_count": count}


@router.post("/datasets/{dataset_id}/folders", status_code=201)
async def create_folder(
    ...
    await dataset_service.get_dataset(db, dataset_id)
    path = await image_service.create_folder(db, dataset_id, body.path)
    return {"path": path}


@router.get(
    "/datasets/{dataset_id}/folders/tree", response_model=list[str]
)
async def get_all_folders(
    ...
    """Return all unique folder paths in a dataset."""
    await dataset_service.get_dataset(db, dataset_id)
    return await image_service.get_all_folder_paths(db, dataset_id)


@router.delete("/datasets/{dataset_id}/folders", status_code=200)
async def delete_folder(
    ...
    count = await image_service.delete_folder(
        db, dataset_id, path, current_user.id, storage
    )
...
@router.post(
    "/datasets/{dataset_id}/folders/batch-delete", status_code=200
)
async def batch_delete_folders(
    ...
    count = await image_service.batch_delete_folders(
        db, dataset_id, body.paths, current_user.id, storage
    )
    return {"deleted_count": count}


@router.patch(
    "/datasets/{dataset_id}/folders/batch-move", status_code=200
)
async def batch_move_folders(
    ...
    count = await image_service.batch_move_folders(
        db, dataset_id, body.paths, body.target_folder
    )
    return {"updated_count": count}
