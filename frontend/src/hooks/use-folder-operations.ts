    ...
    async (folderPath: string) => {
      ...
      try {
        ...
        if (currentPath === folderPath || currentPath.startsWith(folderPath)) {
          ...
          parts.pop()
          callbacks.setCurrentPath(parts.length > 0 ? parts.join('/') + '/' : '')
        } else {
          await callbacks.invalidateFolderContents()
        }
  ...
  const handleCreateFolder = useCallback(
    async (folderPath: string) => {
      ...
      try {
        ...
        await callbacks.showAlert({ title: detail || '폴더 생성에 실패했습니다.' })
        throw e
      }
    },
    [dataStore, callbacks],
  ...
  const handleUpdateFolder = useCallback(
    async (oldPath: string, newPath: string) => {
      ...
      try {
        ...
        const detail = (e as { response?: { data?: { detail?: string } } })
          ?.response?.data?.detail
        await callbacks.showAlert({ title: detail || '폴더 업데이트에 실패했습니다.' })
        throw e
      }
  ...
  const handleFinishRenameInViewer = useCallback(
    async (oldPath: string, newName: string) => {
      ...
      if (trimmed.includes('/') || trimmed.includes('\\')) {
        await callbacks.showAlert({ title: '폴더 이름에 / 또는 \\ 문자를 포함할 수 없습니다.' })
        return
      }
