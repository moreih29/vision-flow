...
- **검증**: 비밀번호 일치 확인, 이메일 형식 검증, 비밀번호 강도 (8자+, 대소문자/숫자/특수문자)
- **완료 후**: 로그인 페이지로 이동
...
- **주요 요소**:
  ...
  - "새 프로젝트" 생성 다이얼로그
  - 프로젝트 편집/삭제
- **카드 정보**: 프로젝트 이름, 설명, DataStore 수, 생성일, 삭제 버튼
- **삭제**: 확인 다이얼로그 후 삭제, 더블 서밋 방지 (deletingRef)
...
### 6. TaskDetailPage (5개 컴포넌트로 분할)
- **목적**: Task의 이미지 및 라벨 클래스 관리
- **구성**: TaskDetailHeader + TaskClassPanel + TaskImageGrid + TaskImageCard + TaskImageListView
- **레이아웃**:
  - 상단: Task 정보 (이름, Task Type, Status, 이미지 수) — `TaskDetailHeader`
  - 좌측: 라벨 클래스 관리 패널 — `TaskClassPanel`
  - 중앙: Task에 포함된 이미지 그리드/리스트 — `TaskImageGrid` + `TaskImageCard`/`TaskImageListView`
- **기능**:
...
- `AppLayout`: 공통 헤더 (Vision Flow 로고 + 유저명 + 로그아웃), 인증된 모든 라우트에 적용
- `ErrorBoundary`: 전역 에러 바운더리 (React class component), App 최상위에 적용
- 로그인/회원가입 페이지에는 레이아웃 미적용
- 네비게이션: URL 기반 (React Router), 뒤로 가기 지원
- 모달 backdrop: 반투명 어둡게만 (blur 효과 없음)
